...
impl SettingsWindow {
	pub fn new(app: Arc<crate::App>, event_loop: &EventLoop<CustomEvent>) -> Result<Self> {
		let visible = !app.args.silent;
		let window = Self::create_window(event_loop, visible)?;
	...
	fn create_window(event_loop: &EventLoop<CustomEvent>, visible: bool) -> Result<Window, OsError> {
		let (screen_w, screen_h) = crate::utils::get_monitor_size();
		let w = (screen_w as f32 * 2.0 / 3.0) as i32;
		let h = (screen_h as f32 * 0.8) as i32;
			...
			webview2::Environment::builder().build(move |env| {
				env.expect("env")
					.create_controller(hwnd, move |controller| {
						...
						w.navigate("http://127.0.0.1:29815/settings.html").expect("navigate");
	...
	pub fn open_add_sounds_dialog(&self, multiselect: bool) -> Vec<std::path::PathBuf> {
		let mut options = wfd::FOS_FILEMUSTEXIST | wfd::FOS_PATHMUSTEXIST;
		if multiselect {
			options |= wfd::FOS_ALLOWMULTISELECT;
		}

		let params = wfd::DialogParams {
			file_types: vec![("MP3 files", "*.mp3"), ("Waveform files", "*.wave;*.wav"), ("Vorbis files", "*.ogg"), ("Free Lossless Audio Codec files", "*.flac"), ("Audio files", "*.mp3;*.wave;*.wav;*.ogg;*.flac"), ("All files", "*.*")],
			options,
			title: "Select sounds",
			owner: Some(self.window_wrapper.window.hwnd() as wfd::HWND),
			..Default::default()
		};

		match wfd::open_dialog(params) {
			Ok(r) => return r.selected_file_paths,
			Err(e) => match e {
				wfd::DialogError::HResultFailed { hresult, error_method } => log::error!("open_add_sounds_dialog failed, HRESULT: {:X}, method: {}", hresult, error_method),
				wfd::DialogError::UnsupportedFilepath => log::error!("open_add_sounds_dialog failed: unsupported file path"),
				wfd::DialogError::UserCancelled => {},
			},
		}
		Vec::new()
	}
...
impl WindowEventHandler for SettingsWindow {
	fn open(&self) {
		self.window_wrapper.window.set_visible(true);
		if let Some(webview_host) = self.window_wrapper.controller.get() {
			webview_host.put_is_visible(true).expect("Could not show SettingsWindow");
		}
	}
	...
	fn on_closed(&self, _control_flow: &mut ControlFlow) {
		...
		if let Some(webview_host) = self.window_wrapper.controller.get() {
			webview_host.put_is_visible(false).expect("Could not hide SettingsWindow");
		}
	}
